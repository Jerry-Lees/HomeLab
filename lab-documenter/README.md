...
2. **Manual Installation**:
   ...
   cp lab-documenter.py /opt/lab-documenter/
   chmod +x /opt/lab-documenter/lab-documenter.py
   ```
...
```bash
# Enable daily runs
sudo systemctl enable --now lab-documenter.timer

# Check timer status
sudo systemctl status lab-documenter.timer

# View recent runs
sudo journalctl -u lab-documenter.service
```
...
0 6 * * * /usr/local/bin/lab-documenter --scan --update-wiki >> /var/log/lab-documenter/cron.log 2>&1
```
...
/opt/lab-documenter/distribute-key.sh user@server1 user@server2

# Or manually copy to each server
ssh-copy-id -i /opt/lab-documenter/.ssh/homelab_key.pub user@server
```
...
/opt/lab-documenter/setup-ssh-agent.sh

# Distribute to multiple servers at once
/opt/lab-documenter/distribute-key.sh admin@192.168.1.100 ubuntu@server.local
```
...
- Store MediaWiki credentials securely (consider environment variables)
- Run the service as the dedicated `lab-documenter` user (not root)
...
ssh -i /opt/lab-documenter/.ssh/homelab_key user@server

# Check SSH key permissions
chmod 600 /opt/lab-documenter/.ssh/homelab_key
chmod 644 /opt/lab-documenter/.ssh/homelab_key.pub
