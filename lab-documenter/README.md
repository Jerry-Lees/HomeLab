...
- **Kubernetes Comprehensive Documentation**: Full cluster resource inventory — StatefulSets, DaemonSets, ingresses, PVs, PVCs, storage classes, ConfigMaps, secrets (names/types in docs), service accounts, roles, cluster roles, role bindings, cluster role bindings, and Helm releases; individual YAML manifests written to `backups/kubernetes/<type>/<namespace>-<name>.yaml` after each scan for targeted recovery (`kubectl apply -f`)
